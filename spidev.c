...
static int spidev_probe(struct spi_device *spi)
{
	...
	pr_info("spidev: spidev_probe called\n");
	...
	pr_info("spidev: spidev_probe called 1\n");
	...
	if (minor < N_SPI_MINORS) {
		...
		pr_info("spidev: spidev_probe creating %lu\n", minor);
...
static int __init spidev_init(void)
{
	...
	pr_info("spidev: spidev_init called\n");
	...
	pr_info("spidev: spidev_init called 1\n");
	...
	pr_info("spidev: spidev_init called 2\n");
	status = spi_register_driver(&spidev_spi_driver);
	pr_info("spidev: spidev_init called 2a status %d\n", status);
	if (status < 0) {
		class_destroy(spidev_class);
		unregister_chrdev(SPIDEV_MAJOR, spidev_spi_driver.driver.name);
	}
	pr_info("spidev: spidev_init called 3\n");
	return status;
}
